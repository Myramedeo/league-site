...
class StandingsTests(TestCase):
    ...
    def test_multiple_games_standings(self):
        ...
        game2 = Game.objects.create(
            season=self.season, competition=self.competition, home_team=self.team_b,
...
class TeamBattingStatsTests(TestCase):
    ...
    def test_aggregates_across_games_and_includes_zero_stat_players(self):
        ...
        Roster.objects.create(player=batter, team=self.team, season=self.season, competition=self.competition)
        Roster.objects.create(player=bench, team=self.team, season=self.season, competition=self.competition)

        opponent = Team.objects.create(name="Owls")
        game1 = Game.objects.create(season=self.season, competition=self.competition, home_team=self.team, away_team=opponent, date='2026-06-01')
        game2 = Game.objects.create(season=self.season, competition=self.competition, home_team=self.team, away_team=opponent, date='2026-06-08')
    ...
    def test_calculates_slugging_and_ops_from_hit_types(self):
        ...
        Roster.objects.create(player=batter, team=self.team, season=self.season, competition=self.competition)
        opponent = Team.objects.create(name="Owls")
        game = Game.objects.create(season=self.season, competition=self.competition, home_team=self.team, away_team=opponent, date='2026-06-01')
        ...
        result = team_batting_stats(self.team, self.competition)[0]
    ...
    def test_player_with_multiple_roster_rows_not_double_counted(self):
        batter = Player.objects.create(first_name='Casey', last_name='Batter')
        other_competition = Competition.objects.create(name='2026 Playoffs', season=self.season)
        Roster.objects.create(player=batter, team=self.team, season=self.season, competition=self.competition)
        Roster.objects.create(player=batter, team=self.team, season=self.season, competition=other_competition)
    ...
    def test_hidden_roster_player_is_excluded_from_team_list_stats(self):
        ...
        Roster.objects.create(player=visible_player, team=self.team, season=self.season, competition=self.competition)
